...
map<const string, weak_ptr<CPMDataReader::Body> > CPMDataReader::bodies_;
static boost::mutex bodies_mutex_;

CPMDataReader::CPMDataReader(const LayerParameter& param)
...
CPMDataReader::QueuePair::QueuePair(int size) {
...
CPMDataReader::QueuePair::~QueuePair() {
  Datum* datum;
...
CPMDataReader::Body::Body(const LayerParameter& param)
    : param_(param),
      new_queue_pairs_() {
  StartInternalThread();
}

CPMDataReader::Body::~Body() {
  StopInternalThread();
}
...
void CPMDataReader::Body::read_one(db::Cursor* cursor, QueuePair* qp) {
